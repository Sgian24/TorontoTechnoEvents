     ...
     <>
        ...
        {dates.filter(j => selectedDate === ""? typeof Date.parse(j) === "number" : Date.parse(j) === selectedDate ).map( k =>  
         ...
         {filteredEvents.filter(i => k === new Date(i.start_time).toDateString()).map(i =>
          <li className="mb-2 ">
            <Row onClick={() => handleShow(i)} className="d-flex justify-content-between border gx-0" style={{height:"30vh", width:"50vw", cursor:"pointer"}}>
                ...
                <Col className="ps-4" xl={9}>
                    ...
                    <div className="fw-bold">{i.venue?.name}</div>
